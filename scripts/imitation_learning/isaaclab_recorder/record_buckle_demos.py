...
"""
Script to record buckle scene demonstrations with manual success marking via SpaceMouse left button.

Based on record_demos.py, but adds user-triggered success marking:
- SpaceMouse left button: mark current episode as successful and save to dataset
- SpaceMouse right button: reset current episode (discard)

Usage:
    python scripts/imitation_learning/isaaclab_recorder/record_buckle_demos.py \
        --task RobotisLab-Buckle-Scene-FFW-BG2-IK-Rel-v0 \
        --teleop_device spacemouse \
        --dataset_file /tmp/buckle_scene.hdf5 \
        --num_demos 0
"""
...
parser = argparse.ArgumentParser(description="Record buckle scene demonstrations with manual success marking.")
...
def process_success_condition(env: gym.Env, success_term: object | None, success_step_count: int) -> tuple[int, bool]:
    ...
    Returns:
        tuple[int, bool]: (updated success_step_count, success_reset_needed)
    """
...
def export_episode_as_success(env: gym.Env):
    """Mark the current episode as successful and export it to the dataset."""
    env.recorder_manager.record_pre_reset([0], force_export_or_skip=False)
    env.recorder_manager.set_success_to_episodes(
        [0], torch.tensor([[True]], dtype=torch.bool, device=env.device)
    )
    env.recorder_manager.export_episodes([0])
    print("Episode marked as successful by user and saved.")
...
def handle_reset(
    ...
    Args:
        ...
        teleop_interface: The teleop device to reset
        success_step_count: Current count of consecutive successful steps
    ...
    # Wait for physics to settle (gripper/object stabilization)
    settle_duration = 1.5  # seconds
    print(f"Waiting {settle_duration}s for physics to settle...")
    settle_end = time.time() + settle_duration
    while time.time() < settle_end:
        env.sim.step(render=True)
        time.sleep(0.016)  # ~60Hz render during settle

    # Re-reset recorder so the settling steps are not recorded
    env.recorder_manager.reset()
    print(">>> Ready! You can start moving now. <<<")
...
def run_simulation_loop(
    ...
    Adds manual success marking via SpaceMouse left button ("L" callback).
    ...
    def mark_success_instance():
        nonlocal should_mark_success
        should_mark_success = True
        print("User triggered success marking (SpaceMouse left button)")
    ...
    # "L" (SpaceMouse left button) -> mark success
    # "R" (SpaceMouse right button) -> reset
    teleoperation_callbacks = {
        "L": mark_success_instance,
    ...
    teleop_interface.add_callback("L", mark_success_instance)
    ...
    # Wait for physics to settle before recording starts
    settle_duration = 1.5  # seconds
    print(f"Waiting {settle_duration}s for initial physics settling...")
    settle_end = time.time() + settle_duration
    while time.time() < settle_end:
        env.sim.step(render=True)
        time.sleep(0.016)  # ~60Hz render during settle
    ...
    print("=== Manual Success Recording Mode ===")
    print("  SpaceMouse left button  : Mark episode as SUCCESS and save")
    print("  SpaceMouse right button : Reset episode (discard)")
    print("=====================================")
    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while simulation_app.is_running():
            ...
            # Check for automatic success condition
            ...
            # Handle manual success marking (SpaceMouse left button)
